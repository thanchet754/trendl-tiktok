...
# 1. Update getClusterProducts to use category-specific diverse image arrays and direct TikTok Shop URLs
NEW_CLUSTER_RESOLVER = '''
...
# Replace old getClusterProducts with NEW_CLUSTER_RESOLVER
old_resolver_marker_start = "// 9.5 Cluster Products & Drawer Interactive Functions"
old_resolver_marker_end = "function toggleClusterDrawer(drawerId, event)"

start_idx = code.find(old_resolver_marker_start)
end_idx = code.find(old_resolver_marker_end)

if start_idx != -1 and end_idx != -1:
    code = code[:start_idx] + NEW_CLUSTER_RESOLVER.strip() + "\n\n        " + code[end_idx:]
    print("Replaced getClusterProducts with TikTok Shop direct URLs and category image sets!")
else:
    print(f"Could not locate getClusterProducts: start={start_idx}, end={end_idx}")

# 2. Update Table Row template so button says `${asinCount} SP TikTok ⌵` and drawer reflects TikTok Shop
code = code.replace("${asinCount} ASINs", "${asinCount} SP TikTok")
code = code.replace("CỤM SẢN PHẨM THỰC TẾ", "CỤM SẢN PHẨM TIKTOK SHOP THỰC TẾ")
code = code.replace("trên Amazon/TikTok", "trên TikTok Shop US")
code = code.replace("products on Amazon / TikTok", "sản phẩm trên TikTok Shop US")
code = code.replace("View all ${asinCount} products", "Xem toàn bộ trên TikTok Shop US")
code = code.replace("Mở Sản Phẩm ↗", "MỞ TIKTOK SHOP ↗")
code = code.replace("${p.asin}", "TikTok Shop")

with open(EXPORTER_PATH, "w", encoding="utf-8") as f:
    f.write(code)

print("Saved updated exporters/html_exporter.py for TikTok Shop!")
